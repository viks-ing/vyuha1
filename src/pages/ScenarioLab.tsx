...
export const ScenarioLab: React.FC = () => {
  ...
  const getScenarioIcon = (id: string) => {
    switch (id) {
      ...
      case 'scn-3':
        return <Anchor className="w-5 h-5 text-purple-400" />;
      case 'scn-4':
        return <DollarSign className="w-5 h-5 text-emerald-400" />;
      case 'scn-5':
        return <Clock className="w-5 h-5 text-rose-400" />;
      default:
        return <FlaskConical className="w-5 h-5 text-sky-400" />;
    }
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div>
        ...
        <h2 className="text-2xl font-bold text-slate-100">Scenario Lab</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {mockScenarios.map((scenario: ScenarioItem) => (
          <Card key={scenario.id} className="flex flex-col justify-between border-slate-800 hover:border-sky-500/40">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-800/80">
                <div className="p-2 rounded-lg bg-slate-900/60 border border-slate-800/80">
                  ...
                  <span className="text-sm font-extrabold text-amber-400">+{scenario.delayImpactDays} Days</span>
                </div>

                <div className="p-2 rounded-lg bg-slate-900/60 border border-slate-800/80">
                  <span className="block text-[10px] text-slate-500 uppercase">Estimated Cost</span>
                  <span className="text-sm font-extrabold text-rose-400">+{formatINR(scenario.costImpactINR)}</span>
                </div>
              </div>
            </CardContent>
      ...
      {selectedScenario && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="glass-card rounded-2xl p-6 max-w-lg w-full border border-sky-500/30 space-y-6 shadow-2xl relative">
            ...
            <div className="space-y-3 bg-slate-900/80 p-4 rounded-xl border border-slate-800">
              <h4 className="text-xs font-bold text-sky-400 uppercase tracking-wider">Affected Operational Vectors</h4>
              <div className="flex flex-wrap gap-2">
                {selectedScenario.affectedFactors.map((factor: string, i: number) => (
                  <span key={i} className="text-xs px-2.5 py-1 rounded-md bg-slate-950 text-slate-200 border border-slate-800">
                    {factor}
                  </span>
                ))}
              </div>
            ...
            <div className="flex justify-between items-center pt-2">
              <span className="text-xs text-slate-400">Ready to run full stress model?</span>

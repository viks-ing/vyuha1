...
3. Multi-Factor Real-Data Calibrated Risk Engine (risk_scorer.joblib) on ORIGINAL late_delivery_risk target
"""
...
def evaluate_models():
    ...
    # 3. MULTI-FACTOR CALIBRATED RISK CLASSIFIER EVALUATION (ORIGINAL late_delivery_risk)
    ...
    print("\n[3] GENUINE CALIBRATED RISK ENGINE (ORIGINAL late_delivery_risk target)")
    ...
    print(f"  Class-1 Precision    : {prec:.4f} | Class-1 Recall: {rec:.4f}")
    ...
    print("-" * 75)
    print(f"{'Delay Prediction Model':<30} | {'MAE / R²':<12} | R² = {te_r2_del:.4f} | {'N/A (Reg)':<10}")
    print(f"{'Logistics Cost Model':<30} | {'MAE / R²':<12} | R² = {te_r2_cost:.4f} | {'N/A (Reg)':<10}")
